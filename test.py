    ...
    for epoch_client_commu in range(args.num_client_commu):
        ...
        if args.algorithm == 0:  # FedAvg without public data
            ...
            for epoch_public_train in range(args.num_public_train):
                ...
                acc__ = eval_model(model=model,
                                   ...
                                   device=args.device)
                acc_.append(acc__)
                message = '  |epoch_public: {}/{}, client: {}/{}, loss: {:.4f}, acc: {:.4f}'.format(
                    epoch_client_train + 1, args.num_client_train, rank, total_rank, loss__[-1], acc__)
                log.info(message)
            ...
            for epoch_public_train in range(args.num_public_train):
                ...
                acc__ = eval_model(model=model,
                                   ...
                                   device=args.device)
                acc_.append(acc__)
                message = '  |epoch_distill: {}/{}, client: {}/{}, loss: {:.4f}, acc: {:.4f}'.format(
                    epoch_client_train + 1, args.num_client_train, rank, total_rank, loss__[-1], acc__)
                log.info(message)
        elif args.algorithm == 4:  # MLL-SGD
            for epoch_client_train in range(args.num_client_train):
                model, loss__ = train_model(model=client_model[rank],
                                            dataloader=train_dataloader[rank],
                                            device=args.device)
        ...
        if rank == 0:
            ...
            for server, clients in enumerate(server_client):
                ...
                server_acc = eval_model(model=server_model[server],
                                        dataloader=test_dataloader,
                                        device=args.device)
