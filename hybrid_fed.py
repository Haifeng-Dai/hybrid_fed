...
parser.add_argument('--dataset', type=str, default='mnist', help='the used dataset.')
parser.add_argument('--alpha', type=int, default=0.5, help='trade-off parameters of distillation.')
parser.add_argument('--T', type=int, default=2, help='temperature of distillation.')
...
parser.add_argument('--distil_way', type=str, default='weighted', help='the way of distillation.')
parser.add_argument('--batch_size', type=int, default=100, help='batch size of trainning.')
parser.add_argument('--num_client_data', type=int, default=1000, help='number of client datas.')
parser.add_argument('--num_server_commu', type=int, default=10, help='number of server communications.')
parser.add_argument('--num_client_commu', type=int, default=10, help='number of clients communicate with servers.')
parser.add_argument('--num_client_train', type=int, default=10, help='number of servers communications.')
parser.add_argument('--model_select', type=int, default=1, help='select the model group.')
...
server_model = list_same_term(num_all_server, model)
            ...
            for client in server_client[server]:
                ...
                for epoch in range(num_client_train):
                    ...
                    acc = eval_model(
                        ...
                        device=device).item()
                    message = '    |{:^15}: {}, acc {:.3f}'.format('local epoch', epoch, acc)
                    log.info(message)
